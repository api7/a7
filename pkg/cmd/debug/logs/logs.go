...
func NewCmdLogs(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "logs",
		Short: "Stream API7 EE logs from Docker or files",
...
func detectContainer() (string, error) {
	cmd := exec.Command("docker", "ps", "--filter", "name=api7", "--format", "{{.Names}}")
	...
	if err != nil {
		...
		if errors.As(err, &exitErr) {
			...
			return "", fmt.Errorf("failed to detect API7 EE containers: %s", stderr)
		}
		return "", fmt.Errorf("failed to detect API7 EE containers: %w", err)
	}
...
func chooseContainer(containers []string) (string, error) {
	if len(containers) == 0 {
		return "", fmt.Errorf("no API7 EE container found. Use --container to specify or --file to tail a log file directly")
	}
	if len(containers) > 1 {
		return "", fmt.Errorf("multiple API7 EE containers found: %s. Use --container to specify one", strings.Join(containers, ", "))
	}
