...
func TestUpdatePluginMetadata_JSON(t *testing.T) {
	ios, _, out, _ := iostreams.Test()
	registry := &httpmock.Registry{}
	registry.Register(http.MethodPut, "/apisix/admin/plugin_metadata/key-auth", httpmock.JSONResponse(`{"id":"key-auth","metadata":{"header":"x-api-key2"}}`))
	...
	opts := &Options{
		...
		GatewayGroup: "gg1",
		PluginName:   "key-auth",
		MetadataJSON: `{"header":"x-api-key2"}`,
	}
