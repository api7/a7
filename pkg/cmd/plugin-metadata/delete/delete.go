...
func actionRun(opts *Options) error {
	...
	if !opts.Force && opts.IO.IsStdinTTY() {
		fmt.Fprintf(opts.IO.ErrOut, "Delete plugin metadata for %q? (y/N): ", opts.PluginName)
	...
	if _, err := client.Delete("/apisix/admin/plugin_metadata/"+opts.PluginName, query); err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	_, err = fmt.Fprintf(opts.IO.Out, "Plugin metadata for %q deleted.\n", opts.PluginName)
	return err
}
