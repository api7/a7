...
func NewCmd(f *cmd.Factory) *cobra.Command {
	...
	c := &cobra.Command{
		Use:   "create",
		Short: "Create a runtime service",
		Args:  cobra.NoArgs,
...
func actionRun(opts *Options) error {
	...
	if opts.File != "" {
		...
		if id, ok := payload["id"]; ok {
			body, err = client.Put(fmt.Sprintf("/apisix/admin/services/%v?gateway_group_id=%s", id, ggID), payload)
		} else {
			body, err = client.Post("/apisix/admin/services?gateway_group_id="+ggID, payload)
		}
		if err != nil {
	...
	client := api.NewClient(httpClient, cfg.BaseURL())
	body, err := client.Post("/apisix/admin/services?gateway_group_id="+ggID, bodyReq)
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	var created api.Service
