...
func TestDeleteService_Success(t *testing.T) {
	...
	registry.Register(http.MethodDelete, "/apisix/admin/services/s1", httpmock.StringResponse(http.StatusOK, `{}`))
	...
	if !strings.Contains(out.String(), `Service "s1" deleted.`) {
		t.Fatalf("unexpected output: %s", out.String())
	}
	registry.Verify(t)
}

func TestDeleteService_MissingGatewayGroup(t *testing.T) {
...
func TestDeleteService_APIError(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	registry := &httpmock.Registry{}
	registry.Register(http.MethodDelete, "/apisix/admin/services/s1", httpmock.StringResponse(http.StatusInternalServerError, `{"message":"boom"}`))
...
func TestDeleteService_ValidationError(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	registry := &httpmock.Registry{}
