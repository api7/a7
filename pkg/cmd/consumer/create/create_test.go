...
func TestCreateConsumer_Success(t *testing.T) {
	...
	registry.Register(http.MethodPost, "/apisix/admin/consumers", httpmock.JSONResponse(`{"username":"alice"}`))
	...
	}, GatewayGroup: "gg1", Username: "alice", Desc: "d"}
	if err := actionRun(opts); err != nil {
		t.Fatalf("actionRun failed: %v", err)
	}
	if !strings.Contains(out.String(), "alice") {
		t.Fatalf("expected consumer output: %s", out.String())
	}
	registry.Verify(t)
}

func TestCreateConsumer_MissingGatewayGroup(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	opts := &Options{IO: ios, Client: func() (*http.Client, error) { return (&httpmock.Registry{}).GetClient(), nil }, Config: func() (config.Config, error) { return &mockConfig{baseURL: "http://api.local"}, nil }, Username: "alice"}
	err := actionRun(opts)
