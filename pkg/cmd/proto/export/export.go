...
func NewCmd(f *cmd.Factory) *cobra.Command {
	...
	c := &cobra.Command{
		Use:   "export",
		Short: "Export protobuf definitions as JSON or YAML",
		Args:  cobra.NoArgs,
...
func actionRun(opts *Options) error {
	...
	if len(items) == 0 {
		fmt.Fprintln(opts.IO.ErrOut, "No protobuf definitions found.")
		return nil
	}
...
func fetchAll(client *api.Client, ggID, label string) ([]api.Proto, error) {
	page := 1
	pageSize := 100
	var all []api.Proto
...
func stripTimestamps(items []api.Proto) []map[string]interface{} {
