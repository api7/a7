...
func TestListCredentials_Table(t *testing.T) {
	ios, _, out, _ := iostreams.Test()
	registry := &httpmock.Registry{}
	registry.Register(http.MethodGet, "/apisix/admin/consumers/alice/credentials", httpmock.JSONResponse(`{"total":2,"list":[{"id":"cred1","desc":"first","plugins":{"key-auth":{}}},{"id":"cred2","desc":"second","plugins":{}}]}`))
	...
	if !strings.Contains(output, "cred1") || !strings.Contains(output, "1") {
...
func TestListCredentials_JSON(t *testing.T) {
	ios, _, out, _ := iostreams.Test()
	registry := &httpmock.Registry{}
	registry.Register(http.MethodGet, "/apisix/admin/consumers/alice/credentials", httpmock.JSONResponse(`{"total":1,"list":[{"id":"cred1","desc":"first"}]}`))

	opts := &Options{IO: ios, Client: func() (*http.Client, error) { return registry.GetClient(), nil }, Config: func() (config.Config, error) {
		return &mockConfig{baseURL: "http://api.local", gatewayGroup: "gg1"}, nil
	}, Output: "json", Consumer: "alice", GatewayGroup: "gg1"}

	if err := actionRun(opts); err != nil {
		t.Fatalf("actionRun failed: %v", err)
	}
	var items []api.Credential
	if err := json.Unmarshal([]byte(out.String()), &items); err != nil {
		t.Fatalf("failed to parse output: %v", err)
	}
	if len(items) != 1 || items[0].ID != "cred1" {
...
func TestListCredentials_MissingGatewayGroup(t *testing.T) {
...
func TestListCredentials_MissingConsumer(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	opts := &Options{IO: ios, Client: func() (*http.Client, error) { return (&httpmock.Registry{}).GetClient(), nil }, Config: func() (config.Config, error) {
		return &mockConfig{baseURL: "http://api.local", gatewayGroup: "gg1"}, nil
	}, GatewayGroup: "gg1"}
...
func TestListCredentials_APIError(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	registry := &httpmock.Registry{}
	registry.Register(http.MethodGet, "/apisix/admin/consumers/alice/credentials", httpmock.StringResponse(http.StatusInternalServerError, `{"message":"boom"}`))
