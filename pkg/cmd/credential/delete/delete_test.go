...
func TestDeleteCredential_Success(t *testing.T) {
	...
	registry.Register(http.MethodDelete, "/apisix/admin/consumers/alice/credentials/cred1", httpmock.StringResponse(http.StatusNoContent, ""))
	...
	if !strings.Contains(out.String(), `Credential "cred1" deleted.`) {
...
func TestDeleteCredential_MissingGatewayGroup(t *testing.T) {
...
func TestDeleteCredential_MissingConsumer(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	opts := &Options{IO: ios, Client: func() (*http.Client, error) { return (&httpmock.Registry{}).GetClient(), nil }, Config: func() (config.Config, error) {
		return &mockConfig{baseURL: "http://api.local", gatewayGroup: "gg1"}, nil
...
func TestDeleteCredential_APIError(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	registry := &httpmock.Registry{}
	registry.Register(http.MethodDelete, "/apisix/admin/consumers/alice/credentials/cred1", httpmock.StringResponse(http.StatusInternalServerError, `{"message":"boom"}`))
