...
func actionRun(opts *Options) error {
	...
	body, err := client.Get("/apisix/admin/ssls", map[string]string{"gateway_group_id": ggID})
	if err != nil {
	...
	if err := json.Unmarshal(body, &resp); err != nil {
		return fmt.Errorf("failed to parse ssl list response: %w", err)
	}
	...
	tp.SetHeaders("ID", "SNIs", "TYPE", "STATUS")
	for _, item := range resp.List {
		tp.AddRow(item.ID, strings.Join(item.SNIs, ", "), item.Type, strconv.Itoa(item.Status))
	}
