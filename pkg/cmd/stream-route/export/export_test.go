...
func TestExport_Success(t *testing.T) {
	...
	registry.Register(http.MethodGet, "/apisix/admin/stream_routes", httpmock.JSONResponse(`{"total":1,"list":[{"id":"sr1","server_port":9000}]}`))
	...
	if !strings.Contains(out.String(), "sr1") {
		t.Fatalf("expected stream route in output, got: %s", out.String())
	}
	registry.Verify(t)
}
...
func TestExport_Empty(t *testing.T) {
	...
	registry := &httpmock.Registry{}
	registry.Register(http.MethodGet, "/apisix/admin/stream_routes", httpmock.JSONResponse(`{"total":0,"list":[]}`))
