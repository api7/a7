...
func actionRun(opts *Options) error {
	...
	if labelValue != "" {
		filtered := make([]api.StreamRoute, 0)
	...
	tp.SetHeaders("ID", "DESCRIPTION", "SERVER_PORT", "SNI")
	for _, item := range resp.List {
		tp.AddRow(item.ID, item.Desc, fmt.Sprintf("%d", item.ServerPort), item.SNI)
	}
