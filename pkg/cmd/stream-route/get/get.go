...
func actionRun(opts *Options) error {
	...
	body, err := client.Get("/apisix/admin/stream_routes/"+opts.ID, query)
	if err != nil {
	...
	tp.AddRow("remote_addr", item.RemoteAddr)
	tp.AddRow("server_addr", item.ServerAddr)
	tp.AddRow("server_port", fmt.Sprintf("%d", item.ServerPort))
	tp.AddRow("sni", item.SNI)
