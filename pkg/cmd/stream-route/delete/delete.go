...
func NewCmd(f *cmd.Factory) *cobra.Command {
	...
	c := &cobra.Command{
		Use:   "delete <id>",
		Short: "Delete a stream (L4) route",
...
func actionRun(opts *Options) error {
	...
	if !opts.Force && opts.IO.IsStdinTTY() {
		fmt.Fprintf(opts.IO.ErrOut, "Delete stream route %q? (y/N): ", opts.ID)
	...
	if _, err := client.Delete("/apisix/admin/stream_routes/"+opts.ID, query); err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	_, err = fmt.Fprintf(opts.IO.Out, "Stream route %q deleted.\n", opts.ID)
	return err
}
