...
func TestGetConsumerGroup_Table(t *testing.T) {
	ios, _, out, _ := iostreams.Test()
	registry := &httpmock.Registry{}
	registry.Register(http.MethodGet, "/apisix/admin/consumer_groups/cg1", httpmock.JSONResponse(`{"id":"cg1","desc":"group-1","plugins":{"a":{},"b":{}}}`))
	...
	}, ID: "cg1", GatewayGroup: "gg1"}

	if err := actionRun(opts); err != nil {
		t.Fatalf("actionRun failed: %v", err)
	}
	output := out.String()
	if !strings.Contains(output, "FIELD") || !strings.Contains(output, "VALUE") || !strings.Contains(output, "group-1") || !strings.Contains(output, "2") {
		t.Fatalf("unexpected table output: %s", output)
	}
	registry.Verify(t)
}

func TestGetConsumerGroup_JSON(t *testing.T) {
	ios, _, out, _ := iostreams.Test()
	registry := &httpmock.Registry{}
	registry.Register(http.MethodGet, "/apisix/admin/consumer_groups/cg1", httpmock.JSONResponse(`{"id":"cg1","desc":"group-1"}`))

	opts := &Options{IO: ios, Client: func() (*http.Client, error) { return registry.GetClient(), nil }, Config: func() (config.Config, error) {
		return &mockConfig{baseURL: "http://api.local", gatewayGroup: "gg1"}, nil
	}, Output: "json", ID: "cg1", GatewayGroup: "gg1"}
...
func TestGetConsumerGroup_MissingGatewayGroup(t *testing.T) {
...
func TestGetConsumerGroup_APIError(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	registry := &httpmock.Registry{}
	registry.Register(http.MethodGet, "/apisix/admin/consumer_groups/cg1", httpmock.StringResponse(http.StatusInternalServerError, `{"message":"boom"}`))
