...
func actionRun(opts *Options) error {
	...
	if opts.File != "" {
		...
		body, err := client.Put("/apisix/admin/consumer_groups/"+opts.ID+"?gateway_group_id="+ggID, payload)
		if err != nil {
	...
	client := api.NewClient(httpClient, cfg.BaseURL())
	body, err := client.Put("/apisix/admin/consumer_groups/"+opts.ID+"?gateway_group_id="+ggID, bodyReq)
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	var updated api.ConsumerGroup
