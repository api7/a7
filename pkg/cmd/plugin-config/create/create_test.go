...
func TestCreatePluginConfig_NotFoundExplainsAPI7EECompatibility(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	registry := &httpmock.Registry{}
	registry.Register(http.MethodPost, "/apisix/admin/plugin_configs", httpmock.StringResponse(http.StatusNotFound, `{"error_msg":"not found"}`))
