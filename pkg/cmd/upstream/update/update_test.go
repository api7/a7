...
func TestUpdateUpstream_Success(t *testing.T) {
	...
	registry.Register(http.MethodPut, "/apisix/admin/upstreams/u1", httpmock.JSONResponse(`{"id":"u1","name":"up2"}`))
	...
	}, ID: "u1", GatewayGroup: "gg1", Name: "up2", Nodes: []string{"127.0.0.1:80=1"}}
	if err := actionRun(opts); err != nil {
		t.Fatalf("actionRun failed: %v", err)
	}
	if !strings.Contains(out.String(), "up2") {
		t.Fatalf("expected updated upstream output: %s", out.String())
	}
	registry.Verify(t)
}

func TestUpdateUpstream_InvalidNode(t *testing.T) {
	...
	opts := &Options{IO: ios, Client: func() (*http.Client, error) { return (&httpmock.Registry{}).GetClient(), nil }, Config: func() (config.Config, error) {
		return &mockConfig{baseURL: "http://api.local", gatewayGroup: "gg1"}, nil
	}, ID: "u1", GatewayGroup: "gg1", Nodes: []string{"bad"}}
	err := actionRun(opts)
