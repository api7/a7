...
func actionRun(opts *Options) error {
	...
	body, err := client.Get("/apisix/admin/upstreams/"+opts.ID, query)
	if err != nil {
	...
	tp.AddRow("type", item.Type)
	tp.AddRow("scheme", item.Scheme)
	tp.AddRow("retries", fmt.Sprintf("%d", item.Retries))
	tp.AddRow("pass_host", item.PassHost)
	tp.AddRow("upstream_host", item.UpstreamHost)
