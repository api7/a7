...
func TestDeleteUpstream_Success(t *testing.T) {
	...
	registry.Register(http.MethodDelete, "/apisix/admin/upstreams/u1", httpmock.StringResponse(http.StatusNoContent, ""))
	...
	}, GatewayGroup: "gg1", ID: "u1"}
	if err := actionRun(opts); err != nil {
		t.Fatalf("actionRun failed: %v", err)
	}
	if !strings.Contains(out.String(), `Upstream "u1" deleted.`) {
		t.Fatalf("unexpected output: %s", out.String())
	}
	registry.Verify(t)
}

func TestDeleteUpstream_MissingGatewayGroup(t *testing.T) {
