...
func NewCmd(f *cmd.Factory) *cobra.Command {
	opts := &Options{IO: f.IOStreams, Client: f.HttpClient, Config: f.Config, Type: "roundrobin", Scheme: "http"}
	c := &cobra.Command{
		Use:   "create",
		Short: "Create a runtime upstream",
		Args:  cobra.NoArgs,
...
func actionRun(opts *Options) error {
	...
	client := api.NewClient(httpClient, cfg.BaseURL())
	body, err := client.Post("/apisix/admin/upstreams?gateway_group_id="+ggID, bodyReq)
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	var created api.Upstream
