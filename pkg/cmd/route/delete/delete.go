...
func actionRun(opts *Options) error {
	...
	if !opts.Force && opts.IO.IsStdinTTY() {
		fmt.Fprintf(opts.IO.ErrOut, "Delete route %q? (y/N): ", opts.ID)
	...
	if _, err := client.Delete("/apisix/admin/routes/"+opts.ID, query); err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	_, err = fmt.Fprintf(opts.IO.Out, "Route %q deleted.\n", opts.ID)
	return err
}
