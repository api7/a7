...
func TestGetRoute_Table(t *testing.T) {
	ios, _, out, _ := iostreams.Test()
	registry := &httpmock.Registry{}
	registry.Register(http.MethodGet, "/apisix/admin/routes/r1", httpmock.JSONResponse(`{"id":"r1","name":"demo","uri":"/demo","status":1}`))
	...
	}, ID: "r1", GatewayGroup: "gg1"}
	if err := actionRun(opts); err != nil {
		t.Fatalf("actionRun failed: %v", err)
	}
	if !strings.Contains(out.String(), "demo") {
		t.Fatalf("expected route name in output: %s", out.String())
	}
	registry.Verify(t)
}

func TestGetRoute_MissingGatewayGroup(t *testing.T) {
