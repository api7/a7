...
func TestStateFilePath_UsesA6ConfigDir(t *testing.T) {
	t.Setenv("A7_CONFIG_DIR", "/tmp/a7-cfg")
	t.Setenv("XDG_CONFIG_HOME", "")
	assert.Equal(t, "/tmp/a7-cfg/update-check.json", StateFilePath())
}

func TestStateFilePath_UsesXDGConfigHome(t *testing.T) {
	t.Setenv("A7_CONFIG_DIR", "")
	t.Setenv("XDG_CONFIG_HOME", "/tmp/xdg")
	assert.Equal(t, "/tmp/xdg/a7/update-check.json", StateFilePath())
}
...
func TestReadWriteState(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("A7_CONFIG_DIR", dir)
	t.Setenv("XDG_CONFIG_HOME", "")

	state := StateFile{
		CheckedAt:     time.Now().UTC().Truncate(time.Second),
		LatestVersion: "v1.2.3",
		LatestURL:     "https://github.com/api7/a7/releases/tag/v1.2.3",
	}
